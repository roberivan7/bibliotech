...
public class Biblioteca extends Cadastrar {
    ...
    public static void acervo(){
        ...
        for (int i = 0; i < livro.size(); i++) {
            System.out.printf("   %s |     %s    |   %s     |   %s   |  %s%n",nameBook.get(i),livro.get(nameBook.get(i)).getFirst(),livro.get(nameBook.get(i)).get(1),livro.get(nameBook.get(i)).get(2),livro.get(nameBook.get(i)).get(3),livro.get(nameBook.get(i)).get(4));
        }
    }
    ...
    public static void buscarLivro(String livro){
        for (String buscar: DadosLivros.getDataLivro().keySet()){
            if (livro.equals(buscar)){
                // Buscar por titulo
                System.out.println("Titulo: "+buscar);
                System.out.println("Autor: "+DadosLivros.getDataLivro().get(buscar).get(0));
                System.out.println("Genero: "+DadosLivros.getDataLivro().get(buscar).get(1));
                System.out.println("Codigo: "+DadosLivros.getDataLivro().get(buscar).get(2));
                System.out.println("Ano: "+DadosLivros.getDataLivro().get(buscar).get(3));
                System.out.println("Quantidade: "+DadosLivros.getDataLivro().get(buscar).get(4)+"\n");
                System.out.println(" ----------------- \n\n");
            }else if(DadosLivros.getDataLivro().get(buscar).contains(livro)){
